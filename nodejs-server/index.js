...
const SmartContractAddress = process.env.SMART_CONTRACT_ADDRESS;
  ...
  const SmartContractAddress1 = process.env.SMART_CONTRACT_ADDRESS1;
  const SmartContractABI1 = [
    {
      "inputs": [],
      "stateMutability": "nonpayable",
      "type": "constructor"
  ...
  const decoder = new InputDataDecoder(SmartContractABI);
  const myContract1 = new web3.eth.Contract(SmartContractABI1, SmartContractAddress1);
  const decoder1 = new InputDataDecoder(SmartContractABI1);
...
//   return user;
// });
let students = require('./students.json'); //            Changed here
...
app.post('/users', async (req, res) => {
  ...
  try{
    let tusers = await Promise.all(tempUser.map(async tu => User.save(tu)));
    // users = [...users, ...tusers];
...
app.put('/students/marks', auth, instructorRoute, async (req, res) => {
...
app.get('/student/instructor/:instructor/submit', auth, instructorRoute, async (req, res) => {
  let resData = [];
  ...
  students.forEach(student => {
    ...
    if(s.length>0){
      s.forEach(subj => {
        resData.push({...student._doc,subjects:subj})
      })
    }
  });
  //console.log(resData);
...
app.post('/login', async (req, res) => {
  ...
  const pojoUser = {}
  pojoUser.name = foundUser.name
  pojoUser.username = foundUser.username
  pojoUser.role = foundUser.role

  foundUser = { ...pojoUser, password: foundUser.password }
  ...
  if (match) {
      // create JWTs
      const accessToken = generateAccessToken(pojoUser)
